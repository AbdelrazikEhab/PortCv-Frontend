...
export default function DevelopSkill() {
    ...
    // Default to current UI language for AI response
    const [responseLang, setResponseLang] = useState(i18n.language);

    // Sync default response lang with UI lang when it changes
    useEffect(() => {
        setResponseLang(i18n.language);
    }, [i18n.language]);
    ...
    return (
        <div className="min-h-screen bg-background p-8">
            <div className="max-w-6xl mx-auto">
                ...
                const {t, i18n} = useTranslation();
                const [responseLang, setResponseLang] = useState(i18n.language); // Default to current UI language

    // Sync default response lang with UI lang when it changes (optional, but good UX)
    useEffect(() => {
                    setResponseLang(i18n.language);
    }, [i18n.language]);
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const selectedResume = resumes.find(r => r.id === selectedResumeId);
                const {data} = await api.post('/ai/career-analysis', {
                    resume: selectedResume?.data,
                language: responseLang // Use the explicitly selected language
            });
                // ...
